...
import com.pm.core.event.NewJobAvailableEvent;
...
public class JobManagementService {
    ...
    public void requestJob(CalJobRequest jobRequest) {
        ...
        applicationEventPublisher.publishEvent(new NewJobAvailableEvent());
    }


    @EventListener(NewJobAvailableEvent.class)
    @Transactional
    public void handleEvent() {
        calculationJobRepository.findFirstByStateForUpdate(CalJobState.READY).ifPresent(job -> {
            ...
            Optional<AgentDevice> agentDevice = agentDeviceRepository.findFirstByStateForUpdate(DeviceState.IDLE);
